...
def eval(model, dataloader, device, criterion):
    ...
    return avg_loss, hamming_loss(test_labels, test_preds)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = train(bert_model, trainloader, device, optimizer, criterion)
        print(f"Epoch {epoch+1} Train Loss: {train_loss:.4f}, Elapsed time: {(time.time() - t0)/60:.2f} minutes")

        test_loss, test_hl = eval(bert_model, testloader, device, criterion)
        print(f"Epoch {epoch+1} Eval Loss: {test_loss:.4f}, Epoch {epoch+1} Eval Hamming Loss: {test_hl:.4f}, Elapsed time: {(time.time() - t0)/60:.2f} minutes")

    torch.save(bert_model.state_dict(), 'models/bert_model_experimental.pth')
